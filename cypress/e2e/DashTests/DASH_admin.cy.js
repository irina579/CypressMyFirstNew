...
{ 
  ...
  beforeEach(() => {
    ...
    it.only('Manage site permissions', () => {
        ...
        cy.contains('.col-sm-6','Local Head of Workforce Planning').siblings('.col-sm-5').should('include.text',',') //verify there are selected users
        cy.get('ul>.select-all').find('label').click() //unselect all
        cy.contains('.col-sm-6','Local Head of Workforce Planning').siblings('.col-sm-5').should('not.include.text',',') //verify users are removed
